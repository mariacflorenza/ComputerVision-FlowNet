...
@torch.no_grad()
def inference(nb_start, nb_end, name_of_the_figure, cuda_device, model, div_flow, path_to_sequesnces):

    flow_dir = Path('my_results') / 'flow'
    ...
    for i in range(nb_start, nb_end):
        ...
        path_img1 = str(Path(path_to_sequesnces) / (name + str(n_img_1).zfill(3) + ".bmp"))
        path_img2 = str(Path(path_to_sequesnces) / (name + str(n_img_2).zfill(3) + ".bmp"))
...
@torch.no_grad()
def complete_inferece_saving_seq(nb_start, nb_end, name, path_to_sequesnces):

    device = set_cuda()
    model, div_flow = load_model(path_to_model, device)

    mask = imread(str(Path(path_to_sequesnces) / (name + "-001.png")))
    ...
    for i in tqdm(range(nb_start, nb_end)):
        ...
        mask_cur = imread(str(Path(path_to_sequesnces) / (name + "-" + str(n_img_2).zfill(3) +".png")))

        img1 = imread(path_img1)
        img2 = imread(path_img2)
        black_image = np.zeros((img1.shape[0], img1.shape[1], 3), dtype=np.uint8)

        # flow = simple_inference(img1, img2, name + str(n_img_1).zfill(3), model, device) # This is line 355, where `flow` is assigned

        # Ensure simple_inference returns the numpy array
        flow = simple_inference(img1, img2, name + str(n_img_1).zfill(3), model, device, save=True)
        ...
        np.save("./my_results/" + name + "-centroid_assessment.npy", centroid_assessment_seq)

    print("ok " + name)        

@torch.no_grad()
def inference_direct(nb_start, nb_end, name_of_the_figure, cuda_device, model, div_flow, path_to_sequesnces):
    ...
    for i in range(nb_start, nb_end):
        ...
        file_name = str(flow_dir / ("direct" + name + "flow-" + str(n_img_1).zfill(3) + "-" + str(n_img_2).zfill(3)))
        rgb_flow = flow2rgb(div_flow*flow_output, max_value=None)
        to_save_rgb = (rgb_flow * 255).astype(np.uint8).transpose(1,2,0)
        imwrite(file_name + '.png', to_save_rgb)
...
@torch.no_grad()
def propagate_mask_direct(nb_start, nb_end, name_in_the_figure, path_to_sequesnces):
    flow_dir = Path('my_results') / 'flow'
    # Ensure the directory exists
    flow_dir.mkdir(parents=True, exist_ok=True)
    original_mask = imread(str(Path(path_to_sequesnces) / (name_in_the_figure + "-001.png")))
    first_img = imread(str(Path(path_to_sequesnces) / (name_in_the_figure + "-001.bmp")))

    for i in range(nb_start+1, nb_end+1):
        flow = np.load( str(flow_dir / ("direct" + name_in_the_figure + "flow-" + "-001-" + str(i).zfill(3) + '.npy')))    
...
def main():
    ...
    global path_to_model  # Declare path_to_model as global
    ...
    if args.mode == 'complete':
        complete_inferece_saving_seq(args.start, args.end, args.sequence, args.sequences_path)
        device = set_cuda()
        model, div_flow = load_model(path_to_model, device)
        inference(args.start, args.end, args.sequence, device, model, div_flow, args.sequences_path)

    elif args.mode == 'direct':
        device = set_cuda()
        model, div_flow = load_model(path_to_model, device)
        inference_direct(args.start, args.end, args.sequence, device, model, div_flow, args.sequences_path)
        propagate_mask_direct(args.start, args.end, args.sequence, args.sequences_path)
...
#         file_name = str(Path("./my_results/") / (name + str(n_img_1).zfill(3) + "-" + str(n_img_2).zfill(3)))
#         rgb_flow = flow2rgb(div_flow*flow_output, max_value=None)
#         to_save_rgb = (rgb_flow * 255).astype(np.uint8).transpose(1,2,0)
#         imwrite(file_name + '.png', to_save_rgb)
...
#         file_name = str(Path("./my_results/") / ("direct" + name + str(n_img_1).zfill(3) + "-" + str(n_img_2).zfill(3)))
#         rgb_flow = flow2rgb(div_flow*flow_output, max_value=None)
#         to_save_rgb = (rgb_flow * 255).astype(np.uint8).transpose(1,2,0)
#         imwrite(file_name + '.png', to_save_rgb)
