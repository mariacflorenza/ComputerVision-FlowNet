...
from models.FlowNetS import FlowNetS  # Import FlowNetS directly
from models.FlowNetC import FlowNetC  # Import FlowNetC if needed
...
parser.add_argument(
    ...
    metavar="PREFIX",
    default=None,
    help="Prefix for image sequences (e.g., 'bear' for bear-001, bear-002, ...)",
)
parser.add_argument(
    "--output",
    "-o",
...
def main():
    ...
    #         if img_pair.is_file():
    #             print("adding file :", file, img_pair)
    ...
    model_arch = "FlowNetS"  # Replace "FlowNetS" with "FlowNetC" if needed

    print(f"=> using pre-trained model '{model_arch}'")
    if model_arch == "FlowNetS":
        model = FlowNetS(batchNorm=False).to(device)  # Initialize FlowNetS
    elif model_arch == "FlowNetC":
        model = FlowNetC(batchNorm=False).to(device)  # Initialize FlowNetC
    else:
        raise ValueError(f"Unknown model architecture: {model_arch}")

    model.load_state_dict(network_data)  # Load the weights
    model.eval()
